...
void main() {
  ...
  group('UX Improvements Verification', () {
    testWidgets('Verify Search Clear Button and YouTube Fallback', (
      tester,
    ) async {
      app.main();
      ...
      expect(
        find.text('Pizza'),
        findsNothing,
        reason: 'Text should be cleared after tapping X',
      );

      // 2. Check YouTube Fallback
      ...
      expect(
        find.text('SUGGESTIONS FROM YOUTUBE'),
        findsOneWidget,
        reason: 'YouTube suggestions should appear if no local recipes match',
      );
    });

    testWidgets('Verify API Key Guard on Home Search Pill', (tester) async {
      app.main();
      await tester.pumpAndSettle();

      // Enter a URL directly into the home search field
      final searchField = find.byType(TextField);
      ...
      expect(
        find.text('Gemini Key Missing'),
        findsOneWidget,
        reason: 'Dialog should appear if importing via URL without an API key',
      );
